...
async def get_my_activity(
    period: str = Query(default="week", description="Aggregation period: 'week', 'month', 'year'"),
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db)
) -> List[DailyActivityItem]:
    """Return calendar activity aggregated metrics for heatmaps and charts."""
    return await progress_service.get_daily_activity(db, user_id=current_user.id, period=period)
...
async def get_my_recent_activity(
    ...
    return await progress_service.get_recent_activity(db, user_id=current_user.id, limit=limit)
...
async def get_my_skills(
    ...
    return await skill_service.get_user_skill_matrix(db, user_id=current_user.id, category=category)
...
async def get_my_skill_detail(
    ...
    return await skill_service.get_user_skill_detail(db, user_id=current_user.id, identifier=skill_id)
