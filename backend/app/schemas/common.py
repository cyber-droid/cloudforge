...
class HealthResponse(BaseModel):
    ...
    """
    status: str = Field(default="ok", description="Process health status ('ok', 'degraded')")
    environment: str = Field(..., description="Current deployment environment (development, staging, production)")
...
class ReadinessResponse(BaseModel):
    ...
    """
    status: str = Field(default="ready", description="Readiness status ('ready', 'unhealthy')")
    environment: str = Field(..., description="Current deployment environment")
    version: str = Field(default="0.1.0", description="Backend service version")
    database: str = Field(..., description="Database connectivity status ('connected', 'error message')")
...
class ErrorResponse(BaseModel):
    ...
    details: Optional[Any] = Field(default=None, description="Detailed validation error list or trace context")
